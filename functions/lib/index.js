...
exports.addReply = exports.likeComment = exports.createDailySetHttp = exports.adminCreateDailySet = exports.submitVote = void 0;
...
exports.submitVote = functions.https.onCall(async (data, context) => {
    ...
    await db.runTransaction(async (tx) => {
        ...
        const updates = {};
        if (optionIndex === 0)
            updates["option0Count"] = inc;
        else
            updates["option1Count"] = inc;
        tx.set(pollRef, updates, { merge: true });
    });
    return { ok: true };
});
exports.adminCreateDailySet = functions.https.onCall(async (data, context) => {
    const uid = context.auth?.uid;
    if (!uid)
        throw new functions.https.HttpsError("unauthenticated", "Login gerekli");
    const dateId = data?.dateId ?? todayIdUTC();
...
exports.createDailySetHttp = functions.https.onRequest(async (req, res) => {
    const dateId = todayIdUTC();
    const dailyRef = db.collection("daily_sets").doc(dateId);
